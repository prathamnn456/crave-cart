...
const StarRating = ({ rating = 0, count }) => {
  const isNew = !count; // no reviews yet
  const r = isNew ? 0 : rating;
  const pct = Math.max(0, Math.min(100, (r / 5) * 100));
  return (
    <span className='star-rating' title={isNew ? 'No ratings yet' : `${r} out of 5`}>
      <span className='star-rating-stars'>
        <span className='star-rating-bg'>★★★★★</span>
        <span className='star-rating-fg' style={{ width: pct + '%' }}>★★★★★</span>
      </span>
      {isNew ? <b className='star-rating-new'>New</b> : <b>{r.toFixed(1)}</b>}
      {!isNew && count != null && <small>({count})</small>}
    </span>
  )
}
